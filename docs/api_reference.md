...
```json
{
  ...
  "contexto": "O lead deseja saber mais sobre as soluções de automação da MindFlow."
}
```
...
| `contexto` | `string` | `""` | Contexto ou informações adicionais para personalizar a chamada. Injetado diretamente no prompt dinâmico. |
...
curl -X POST https://call-github.bkpxmb.easypanel.host/webhook \
  ...
  -d '{
    ...
    "contexto": "Lead de alta prioridade capturado via formulário."
  }'
```
