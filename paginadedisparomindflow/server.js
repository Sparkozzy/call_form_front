...
app.get('/api/call-status/:executionId', async (req, res) => {
  ...
  try {
    ...
    // 2. Busca da tabela Retell_calls_Mindflow
    if (callId) {
    ...
    // 3. Fallback: Se não achou no Supabase e temos API Key da Retell, busca ao vivo na Retell
    if (callId && !call && process.env.RETELL_API_KEY) {
      try {
        ...
        if (retellRes.ok) {
          const rCall = await retellRes.json();
          call = {
            ...
            transcript_object: rCall.transcript_object || rCall.transcript_with_tool_calls || null,
            public_log_url: rCall.public_log_url || null,
            recording_url: rCall.recording_url || '',
            created_at: rCall.start_timestamp ? new Date(rCall.start_timestamp).toISOString() : null
          };
        }
      } catch (rErr) {
        console.warn('[CallStatus] Fallback Retell API erro:', rErr.message);
      }
    }
    ...
    if (call) {
      ...
      if (reason !== '' || ['ended', 'completed', 'error'].includes(callStatus)) {
        ...
        } else if (['dial_no_answer', 'no-answer', 'no_answer', 'voicemail_reached'].includes(reason)) {
          stage = 'NO_ANSWER';
        ...
        stage = 'RINGING';
        stageLabel = 'Discando para o telefone...';
      }
    } else if (execution) {
